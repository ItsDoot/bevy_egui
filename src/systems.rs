...
use bevy::{
    ...
    log::{self, error},
    prelude::{Commands, Entity, EventReader, NonSend, Query, Resource, Time},
...
pub fn process_output_system(
    ...
    for mut context in contexts.iter_mut() {
        ...
        if let Some(mut window) = context.window {
            let mut set_icon = || {
                let icon = egui_to_winit_cursor_icon(platform_output.cursor_icon)
                    .unwrap_or(bevy::window::SystemCursorIcon::Default);
                commands
                    .entity(context.render_target)
